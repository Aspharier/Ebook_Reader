...
class LibraryViewModel @Inject constructor(private val repository: BookRepository) : ViewModel() {
            ...
            repository
                    ...
                    .map { books ->
                        if (books.isEmpty()) {
                            ...
                            val categorizedBooks = books.groupBy { it.category }
                            LibraryUiState.Success(books, categorizedBooks)
                        }
                    }
    ...
    fun importPdfs(context: Context, uris: List<Uri>, category: String = "Other") {
        viewModelScope.launch(Dispatchers.IO) {
            uris.forEach { uri ->
                try {
                    importSinglePdf(context, uri, category)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
    ...
    private suspend fun importSinglePdf(context: Context, uri: Uri, category: String) {
        val fileName = queryFileName(context, uri) ?: "Untitled.pdf"
        val bookId = UUID.randomUUID().toString()

        val pdfFile = File(context.filesDir, "$bookId.pdf")
    ...
    private suspend fun processAndUpdateBook(
        ...
        val coverPath = result.coverBitmap?.let { saveCoverBitmap(context, baseBook.id, it) }

        val updatedBook = baseBook.copy(totalPages = result.pageCount, coverImagePath = coverPath)
